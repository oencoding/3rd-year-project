...
if __name__ == '__main__':
	...
	if not USE_QUEUE_LOADING:
		...
		#A FIFOQueue holds a list of up to capacity elements. Each element is a fixed-length tuple of tensors whose dtypes are described by dtypes, 
		#and whose shapes are optionally described by the shapes argument.
        #If the shapes argument is specified, each component of a queue element must have the respective fixed shape. 
		#If it is unspecified, different queue elements may have different shapes, but the use of dequeue_many is disallowed.
	...
	gvs = zip(tf.gradients(loss,tvars), tvars)#optimizer.compute_gradients
	norm = BASE_NORM*BASE_LR/lr
	#tf.clip_by_norm(g, clip_norm, axes=None, name=None)
	#Given a tensor g, and a maximum clip value clip_norm, this operation normalizes g so that its L2-norm is less than or equal to clip_norm, along the dimensions given in axes.
	#Clips tensor values to a maximum L2-norm.Particularly useful for handling exploding or vanishing gradients.
	#If the L2-norm is greater than clip_norm, then this operation returns a tensor of the same type and shape as g with its values set to:
	#g * clip_norm / l2norm(g)
	...
	with tf.Session(config=config) as sess:
		...
		if USE_QUEUE_LOADING:
			...
			for epoch in range(0, MAX_EPOCH):
				...
				for i in range(num_thread):
					length = len(train_list)/num_thread
					#multi thread to load the data,比如有200个data和20个thread，第1个thread加载1-10，第二个thread加载11-20
