...
tf.app.flags.DEFINE_string('dataset', 'Set5',
...
tf.app.flags.DEFINE_integer('train_time', 560,
...
def _train():
    ...
    # Create and initialize model
	#features -> G -> gene_output -> D -> disc_fake_output
	#labels -> D -> disc_real_output
    [gene_minput, gene_moutput,
     gene_output, gene_var_list,
     disc_real_output, disc_fake_output, disc_var_list] = \
            srez_model.create_model(sess, noisy_train_features, train_labels)
	#output of generator is the distribution of data not label,
	#output of discriminator is label,gene_loss is to measure the performance of generator
