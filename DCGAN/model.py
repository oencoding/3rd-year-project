...
class Model:
    ...
    def add_conv2d(self, num_units, mapsize=1, stride=1, stddev_factor=1.0):
        ...
        with tf.variable_scope(self._get_layer_str()):
			...
			#then get the number of channels in that layer
            prev_units = self._get_num_inputs()
    ...
    def add_upscale(self):
        ...
        prev_shape = self.get_output().get_shape()
        size = [2 * int(s) for s in prev_shape[1:3]]#width and height upscaled by 2
...
def _discriminator_model(sess, features, disc_input):
    ...
    model.add_conv2d(1, mapsize=1, stride=1, stddev_factor=stddev_factor)#1 channel left
    model.add_mean()#find average ,64*64 to a single value
...
def _generator_model(sess, features, labels, channels):
    ...
    for ru in range(len(res_units)-1):
        ...
        model.add_upscale()#[b,16,16,256] to[b,32,32,256]
        
        model.add_batch_norm()
        model.add_relu()
		#[b,32,32,256] conv with [3,3,256,256]
	...
	# the final output will be [b,64,64,3]
...
def create_model(sess, features, labels):
	...
	#gene_minput is for test feature input
    ...
    with tf.variable_scope('gene') as scope:
		# label are not used in the generator model,only features are used
		# gene_output is output from the generator
        gene_output, gene_var_list = \
                    _generator_model(sess, features, labels, channels)

        scope.reuse_variables()
        #gene_moutput is for test feature output from generator
    ...
    with tf.variable_scope('disc') as scope:
		#features are not used in the discriminator model,only label are used
		#First use discriminator model to sample real data
        disc_real_output, disc_var_list = \
                _discriminator_model(sess, features, disc_real_input)

        scope.reuse_variables()
        #Second  use discriminator model to sample generator model data
        disc_fake_output, _ = _discriminator_model(sess, features, gene_output)
	
    print(gene_output.get_shape()) 
    print(disc_fake_output.get_shape()) 
    print(disc_real_output.get_shape()) 
...
def create_generator_loss(disc_output, gene_output, features):
	...
	#bigger disc_fake_output,smaller the cross_entropy,optimize G and D to get minize cross_entropy
	#we want smaller cross_entropy so that disc_output is bigger, more likely to be classfied as data
    ...
    #calcualte difference between distribution of generator and  training example,which we shuold minize
    ...
    return gene_loss
#we want disc_fake_output to be as big as possible also we want it to be as small as possible when compare pg and px
#so there is a trade off between disc_fake_output and disc_real_output
def create_discriminator_loss(disc_real_output, disc_fake_output):
    # I.e. did we correctly identify the input as real or not?
	#bigger disc_real_output,smaller the cross_entropy_real
    cross_entropy_real = tf.nn.sigmoid_cross_entropy_with_logits(disc_real_output, tf.ones_like(disc_real_output))
    disc_real_loss     = tf.reduce_mean(cross_entropy_real, name='disc_real_loss')
    #smaller disc_fake_output,the smaller cross_entropy_fake
